...
-- Conversion Percentage of different shots across seasons
...
--  Conversion Comparison of different shots for all the leagues using PIVOT

SELECT situation, [Premier League],[La Liga],[Serie A],[Bundesliga],[Ligue 1]
FROM
(SELECT
distinct(leagues.name), shots.situation,
round(cast(sum(case when shots.shotResult = 'Goal' then 1 else 0 end) over (Partition by leagues.name, shots.situation) as float)/cast(count(shots.shotResult) over (Partition by leagues.name, shots.situation) as float)*100,1) as ConversionPerc
FROM Shots
inner join Games ON games.gameID = shots.gameID
inner join Leagues ON leagues.leagueID = games.leagueID
)
as SourceTable
PIVOT
(
min(ConversionPerc)
FOR SourceTable.Name IN ([Premier League],[La Liga],[Serie A],[Bundesliga],[Ligue 1])
)as Pvt;